...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path):
    ...
    train_y = one_hot_vector(label[train_indices], 11)
    test_y = one_hot_vector(label[test_indices], 11)
    
    train_p_y = one_hot_vector_performer(performer[train_indices]-1, 40)
    test_p_y = one_hot_vector_performer(performer[test_indices]-1, 40)
...
if __name__ == '__main__':
    ...
    label = np.loadtxt(label_file, dtype=np.int)  # action label: 0~59

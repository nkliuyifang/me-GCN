...
if __name__ == "__main__":
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub' in arg.dataset:
            # /data/LMY/ntu_interaction/ntu120/NTU120_CSub.npz
            npz_data = np.load('/data/LMY/ntu_interaction/' + 'ntu120/' + 'NTU120_CSub.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xset' in arg.dataset:
            npz_data = np.load('/data/LMY/ntu_interaction/' + 'ntu120/' + 'NTU120_CSet.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
